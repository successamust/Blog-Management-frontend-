const DEFAULT_SITE_URL = process.env.SITE_URL || process.env.NEXT_PUBLIC_SITE_URL || process.env.VITE_SITE_URL || 'https://thenexusblog.vercel.app';
const DEFAULT_API_BASE =
  process.env.API_BASE_URL || process.env.VITE_API_BASE_URL || 'https://blog-management-sx5c.onrender.com/api';
...
const fetchPostBySlug = async (slug) => {
  const url = `${DEFAULT_API_BASE.replace(/\/$/, '')}/posts/${encodeURIComponent(slug)}`;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const description = escapeHtml(
      post.excerpt || post.summary || post.metaDescription || stripHtml(post.content).slice(0, 180) || FALLBACK_DESCRIPTION
    );
